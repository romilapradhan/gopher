...
\begin{document}
    \begin{tikzpicture}[scale=1, every node/.style={scale=0.75}]
        \node (1) [fill=red!20,draw, rounded rectangle, line width = 0.4mm] {Rank $=$ Full Professor};
        \node (2) [right=of 1, xshift=-12.5mm] {$\wedge$};
        \node (3) [fill=white!20, draw, right=of 2, xshift=-12.5mm, rounded rectangle, line width = 0.4mm] {Years in current role $\in [10, 15]$};
