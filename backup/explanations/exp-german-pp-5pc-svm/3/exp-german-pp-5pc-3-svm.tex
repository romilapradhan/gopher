...
\begin{document}
    \begin{tikzpicture}[scale=1, every node/.style={scale=0.75}]
        ...
        \node (3) [fill=green!20, draw, right=of 2, xshift=-12.5mm, rounded rectangle] {Gender $=$ Male};
        \node (4) [right=of 3, xshift=-12.5mm] {$\wedge$};
        \node (5) [fill=yellow!20, draw, right=of 4, xshift=-12.5mm, rounded rectangle] {Savings $<0$ DM};
        \node (6) [right=of 5, xshift=-12.5mm] {$\wedge$};
        \node (7) [fill=teal!20, draw, right=of 6, xshift=-12.5mm, rounded rectangle] {Purpose $=$ Car (new)};
    \end{tikzpicture}
