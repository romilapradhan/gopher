...
def logistic_loss_torch(model, x, y_true, reg=1):
    if isinstance(y_true, int) or isinstance(y_true, np.ndarray):
        ...
        y_pred = model(x)
        return reg*model.C * torch.norm(convert_grad_to_tensor(list(model.parameters())), p=2)**2+binary_cross_entropy(y_pred, y_true)
...
def svm_loss_torch(model, x, y_true):
    ...
    return model.C * torch.norm(convert_grad_to_tensor(list(model.lr.parameters())), p=2)+torch.mean(model.smooth_hinge(1 - y_true*model.decision_function(x)))
