...
\begin{document}
	...
	\node (1) [fill=blue!20,draw, rounded rectangle] {Gender $=$ Male};
	\node (2) [right=of 1, xshift=-12.5mm] {$\wedge$};
	\node (3) [fill=yellow!20, draw, right=of 2, xshift=-12.5mm, rounded rectangle, line width=0.4mm] {Relationship $=$ Not married};
	\node (4) [right=of 3, xshift=-12.5mm] {$\wedge$};
	\node (5) [fill=white!20, draw, right=of 4, xshift=-12.5mm, rounded rectangle] {Hours $<40$};
     \node (6) [right=of 5, xshift=-12.5mm] {$\wedge$};
     \node (7) [fill=white!20, draw, right=of 6, xshift=-12.5mm, rounded rectangle, line width=0.4mm] {Workclass $=$ Self-emp-not-inc};
	\end{tikzpicture}
