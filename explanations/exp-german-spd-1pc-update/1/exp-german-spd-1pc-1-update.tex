...
\begin{document}
    \begin{tikzpicture}[scale=1, every node/.style={scale=0.75}]
        \node (1) [fill=red!20,draw, line width=0.4mm, rounded rectangle] {Age $<45$};
        \node (2) [right=of 1, xshift=-12.5mm] {$\wedge$};
        \node (3) [fill=white, draw, right=of 2, xshift=-12.5mm, rounded rectangle] {Credit amount $\in [2000, 5000]$};
        \node (4) [right=of 3, xshift=-12.5mm] {$\wedge$};
        \node (5) [fill=white, draw, right=of 4, xshift=-12.5mm, line width=0.4mm, rounded rectangle] {Purpose $=$ Car/Radio};
    \end{tikzpicture}
