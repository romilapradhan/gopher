...
\begin{document}
    \begin{tikzpicture}[scale=1, every node/.style={scale=0.75}]
        ...
        \node (3) [fill=blue!20, draw, right=of 2, xshift=-12.5mm, rounded rectangle] {Degree $=$ Doctorate};
        \node (4) [right=of 3, xshift=-12.5mm] {$\wedge$};
        \node (5) [fill=white!20, draw, right=of 4, xshift=-12.5mm, rounded rectangle] {Years in current role $\in[10, 15]$};
    \end{tikzpicture}
\end{document}