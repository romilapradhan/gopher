...
\begin{document}
    \begin{tikzpicture}[scale=1, every node/.style={scale=0.75}]
        \node (1) [fill=red!20,draw, rounded rectangle, line width=0.4mm] {Age$< 45$};
        \node (2) [right=of 1, xshift=-12.5mm] {$\wedge$};
        \node (3) [fill=white!20, draw, right=of 2, xshift=-12.5mm, rounded rectangle] {Status $\in [0, 200]$};
        \node (4) [right=of 3, xshift=-12.5mm] {$\wedge$};
        \node (5) [fill=white!20, draw, right=of 4, xshift=-12.5mm, rounded rectangle, line width=0.4mm] {Installment rate $=4$};
    \end{tikzpicture}
