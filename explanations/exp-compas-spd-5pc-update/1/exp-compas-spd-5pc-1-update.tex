...
\begin{document}
    \begin{tikzpicture}[scale=1, every node/.style={scale=0.75}]
        ...
        \node (3) [fill=red!20, draw, right=of 2, xshift=-12.5mm, rounded rectangle, line width=0.4mm] {Age $\in [25, 45]$};
        \node (4) [right=of 3, xshift=-12.5mm] {$\wedge$};
        \node (5) [fill=yellow!20, draw, right=of 4, xshift=-12.5mm, rounded rectangle, line width=0.4mm] {Event $=1$};
        \node (6) [right=of 5, xshift=-12.5mm] {$\wedge$};
        \node (7) [fill=white, draw, right=of 6, xshift=-12.5mm, rounded rectangle] {Race $=$ Non-Caucasian};
    \end{tikzpicture}
